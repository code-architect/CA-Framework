...
$errorName = __( 'Please enter your name.', 'architect' );
$errorEmail = __( 'Please enter a valid email address.', 'architect' );
$errorMessage = __( 'Please enter the message.', 'architect' );
...
if ( isset( $_POST['is-submitted'] ) ) {
    ...
    if ( ! $isError ) {
        ...
        $emailSubject = sprintf( __( 'You have been contacted by %s', 'architect' ), $name );
        $emailBody    = sprintf( __( 'You have been contacted by %1$s. Their message is:', 'architect' ), $name ) . PHP_EOL . PHP_EOL;
        $emailBody    .= $message . PHP_EOL . PHP_EOL;
        $emailBody    .= sprintf( __( 'You can contact %1$s via email at %2$s', 'architect' ), $name, $email );
                ...
                <div class="entry-content">
                    <?php if ( isset( $emailIsSent ) && $emailIsSent ) : ?>
                        <div class="alert alert-success">
                            <?php _e( 'Your message has been sucessfully sent, thank you!', 'architect' ); ?>
                        </div> <!-- end alert -->
                    <?php else : ?>
                        ...
                        <?php if ( isset( $isError ) && $isError ) : ?>
                            <div class="alert-alert-danger">
                                <?php _e( 'Sorry, it seems there was an error.', 'architect' ); ?>
                            </div> <!-- end alert -->
                        <?php endif; ?>
                    <?php endif; ?>
                    ...
                    <form action="<?php the_permalink(); ?>" id="contact-form" method="POST" role="form">
                        <div class="form-group <?php if ( isset( $errors['errorName'] ) ) echo "has-error"; ?>">
                            <label for="contact-name" class="control-label"><span class="required">* </span><?php _e( 'Name:', 'architect' ); ?></label>
                        ...
                        <div class="form-group <?php if ( isset( $errors['errorEmail'] ) ) echo "has-error"; ?>">
                            <label for="contact-email" class="control-label"><span class="required">* </span><?php _e( 'Email Address:', 'architect' ); ?></label>
                        ...
                        <div class="form-group <?php if ( isset( $errors['errorMessage'] ) ) echo "has-error"; ?>">
                            <label for="contact-message" class="control-label"><span class="required">* </span><?php _e( 'Message:', 'architect' ); ?></label>
                        ...
                        <button type="submit" class="btn btn-default"><?php _e( 'Send Message', 'architect' ); ?></button>
                    </form>
